...
**MOBILE AUDIT + NAV FIX (2026-08-23).** Context: week to 23 Aug = 140 visitors (+46%), **86% Australian, 84% mobile, 66% iOS**, but bounce climbed to **89%** and only 4 of 140 reached /job, 1 reached /review. Traffic is now overwhelmingly Instagram (53) + Facebook (41); Google down to 11 since the ads were paused — i.e. high-volume low-intent social has replaced high-intent search. STILL ZERO completions ever: 0 live Stripe orders, 0/32 promo links redeemed, coupon 0/100, no checkout activity since Evren's own 8 Aug tests.
ROOT CAUSE FOUND by Playwright device audit against production: the desktop nav (logo + 4 anchor links + CTA) **wrapped onto three rows on a phone = 178px, ~24% of the viewport**; with the promo banner that was ~37% of the first screen as chrome, pushing the headline to 447px and the hero CTA to 776px (below a 751px fold). FIXED: `.swNavLinks` hidden under 760px (they are in-page anchors, redundant on mobile), `.swNavBar`/`.swNavCta` tightened, `.swHero` padding 72/84 → 24/40 on phones. RESULT: nav 178→**67px**, headline 447→335px, hero CTA 776→699px (now peeking into view; not fully above fold on iPhone SE — the promo banner + deliberately huge headline eat the rest). Page is still **~18 phone screens** long.
NOTABLE: the scroll-build section is NOT the bounce culprit — real iPhone viewports are ~664-751px tall, which trips its `max-height:699px` static fallback, so most iOS visitors never see the pinned version at all. Worth knowing before investing more in it.
STILL UNMEASURED: where inside the builder people abandon (generation takes ~85s — prime suspect). Vercel Analytics custom events are Pro-only, so funnel tracking needs a different approach.

**GOOGLE ADS PAUSED 2026-08-18 — spend with nothing to show.** Final numbers 9-18 Aug: 1,102 impressions, 42 clicks, **A$2.35 avg CPC, A$98.55 campaign cost, 0 conversions**. Billing: August net cost A$108.39, only A$15.00 actually charged (12 Aug threshold), **A$93.39 outstanding** — would have auto-charged at the A$100 balance threshold within ~a day, hence pausing now. Note the shape: only A$1.34 had been spent by 11 Aug; the Maximise-Clicks bid fix unblocked delivery and it then burned the full A$15/day cap for a week. DECISIVE EVIDENCE it is not a pricing problem: the product was FREE for the final two days and still nobody completed — 42 paid clicks, zero finished builders. At A$2.35/click on a $7.99 product you need ~1-in-3 to buy just to break even, which never happens. Campaign is PAUSED (not removed) — keywords, negatives, RSA and conversion tracking all intact for a restart once the funnel actually converts. OUTSTANDING: advertiser verification deadline is **2026-09-12**; ads may be paused/limited if not completed, and it must be done before restarting.

**PAYWALL WAS OPEN — FIXED (2026-08-16). Most important finding of the session.** `POST /api/download/preview` returned the finished, UNWATERMARKED SWMS PDF to anyone who posted the payload: no payment check, no token, no rate limit, and `watermark` was caller-controlled and defaulted to OFF. `POST /api/download/[token]` was a second copy of the same hole that ignored its own token segment. The paywall had only ever been client-side — since launch, anyone who opened devtools on the checkout page could take the paid product. Found by a 39-agent adversarial review, not by normal testing. FIX: `src/lib/utils/entitlement.ts` — unwatermarked renders now require proof of purchase, either a settled Stripe session (`payment_status` paid OR `no_payment_required`, so promo orders pass) or a purchase token; `tokenBelongsToOrder()` deliberately ignores `used` so re-downloads keep working after a credit is spent. Watermarked previews stay open. Rate limit 30/hr/IP added. The dead POST handler on the [token] route was deleted rather than secured. Both call sites now send credentials: success page → `session_id`, builder checkout → `token`. GOTCHA HIT: the builder clears `redemptionToken` from the store the moment the credit is spent, which broke the Done-screen retry button — `paidTokenRef` now preserves it. VERIFIED on production: attacker 402, real token 200 + 81KB PDF, watermarked preview 200.

**FREE LAUNCH PROMO (2026-08-16, LIVE).** Free SWMS until 31 Aug or 100 redemptions, whichever first. Mechanism: a 100%-off Stripe coupon `LAUNCHFREE` (live + test mode, same id) applied to the Checkout Session — Stripe itself enforces cap and expiry, and a $0 session skips card collection entirely (verified: live checkout shows only an email field and "Complete order"). Env: `STRIPE_PROMO_COUPON=LAUNCHFREE` + `NEXT_PUBLIC_PROMO_FREE=1` (both set in Vercel production). **TO END THE PROMO:** remove both env vars and redeploy; the coupon also dies on its own at the cap/expiry.
CRITICAL DESIGN POINT — why it is not a bait-and-switch: `NEXT_PUBLIC_*` is inlined at build time, so the UI *cannot* know the coupon has been exhausted. That made "advertise free, then charge" the guaranteed end state, not an edge case. So: `GET /api/promo` reports the coupon's real `valid` state from Stripe (60s cache) and both the landing page and builder checkout correct themselves on mount; and `/api/checkout` takes `expect_free` — if the buyer was shown FREE and the coupon has died, it returns **409 promoEnded** instead of a paid session, and the page tells them the offer ended. Only genuine coupon faults (`coupon_expired`, `resource_missing` on coupon) trigger the downgrade path; any other Stripe error is a plain 500 so a transient blip never costs a promo-eligible buyer $7.99.
OTHER BUG THIS SURFACED: `session.amount_total || 799` in the webhook — 0 is falsy, so free orders were emailing "thanks for your purchase of $7.99". Changed to `??` in both email paths. Watch for `||` on any amount field.
Copy reconciled for the free run across landing (band, hero trust line, pricing, FAQ CTA, final CTA), builder checkout, success page ($0 shows "FREE — LAUNCH OFFER" and drops the receipt promise), SEO shell CTA band, and the site meta description. Delivery emails ask for honest feedback + permission to quote — deliberately NOT a review-for-product trade (platform policies ban incentivised reviews; ACCC is active on this).
